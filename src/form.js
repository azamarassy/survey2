    ...
    forms.push(
      <form key={i} className="form-horizontal" action="/" method="POST" name={`assumption${i}`}>
        <div className="form-group row ms-1 my-3">
          <label className="col-sm-4 control-label ml-3" style={{ fontSize: '20px' }} htmlFor={`asset${i}`}>名前</label>
          <div className="col-sm-4">
            <input type="text" name={`asset_name${i}`} className="form-control" onChange={restrictInputToNumbers}/>
          </div>
          <div className="col-sm-4 d-flex align-items-center">
            <input type="submit" className="submitButton btn btn-primary mr-2" style={{ fontSize: '20px' }} value="決定" onClick={getValue}/>
            <input type="reset" className="clearButton btn btn-primary" name={`clear${i}`} style={{ fontSize: '20px', marginLeft: '4px' }} value="クリア" onClick={(event) => clearForm(event, i)}/>
          </div>
        </div>
      </form>
    );
